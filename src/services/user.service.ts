...
export const findUserById = async (id: string, selectFields?: UserSelect): Promise<User | null> => {
  return await userDal.findUserById(id, selectFields || defaultSelectFields) as User | null;
};
...
export const updateUserPasswordAndClearCode = async (id: string, passwordHash: string) => {
...
export const changePassword = async (id: string, data: ChangePasswordDto) => {
  ...
  if (!(await comparePassword(data.oldPassword, user.passwordHash))) {
    throw new UnauthorizedError({ message: 'Invalid old password', code: 'VALIDATION_ERROR' });
  }
