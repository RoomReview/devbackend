...
export const confirmCheckout = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const data = await paymentService.confirmCheckout(req.params.sessionId as string, req.user!.userId);
  const response: ApiResponse<typeof data> = { success: true, statusCode: 200, data, message: 'Checkout status fetched successfully' };
  res.status(200).json(response);
};